...
export function biliApiProxy(): Plugin {
  return {
    ...
    configureServer(server) {
      const biliFetch = async (url: string) => {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          if (req.url.startsWith('/api/bili/profile')) {
            ...
            const raw = await biliFetch(`https://api.bilibili.com/x/web-interface/card?mid=${mid}`)
            ...
            const cardRaw = await biliFetch(`https://api.bilibili.com/x/web-interface/card?mid=${mid}&photo=true`)
            ...
            const raw = await biliFetch(`https://api.bilibili.com/x/space/acc/info?mid=${mid}&jsonp=jsonp`)
            ...
            const raw = await biliFetch(`https://api.bilibili.com/x/series/recArchivesByKeywords?mid=${mid}&keywords=&ps=${ps}&pn=1`)
            ...
            const items = await Promise.all(
              bvidList.map(async (bvid) => {
                const raw = await biliFetch(`https://api.bilibili.com/x/web-interface/view?bvid=${bvid}`)
                const v = raw?.data ?? {}
                return {
                  bvid: v.bvid,
                  aid: v.aid,
